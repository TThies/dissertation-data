...
# Analysis of tongue body movements comparing healthy controls and speakers with PD
...
DUR_stats2 %>% 
  ggplot(aes(x=med_con, y=M, group = focus_con)) + 
  geom_line(linetype="dashed", color="gray", size=2)+
  geom_point(size = 14, aes(color = focus_con))+
  theme_classic() +
  ...
  coord_cartesian(ylim = c(155, 205)) + 
  scale_y_continuous(breaks = seq(160, 200, 10)) +
  ylab("Duration (ms)")+
...
mdl <- lmer(GAI_V1 ~ med_con + focus_con + vowel + cons + age  + (1 + focus_con|speaker), data_red, REML = FALSE)
...
DISPL_stats2 %>% 
  ...
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 36))+
  scale_color_manual(values = c("#8da0cb", "#66c2a5", "#fc8d62")) +
  coord_cartesian(ylim = c(6.3, 8.2)) + 
  scale_y_continuous(breaks = seq(6.5, 8, 0.5)) +
  xlab(NULL) +
  scale_x_discrete(labels = c('CON','PD'))+
  ylab("Amplitude (mm)")+
  theme(axis.title.y = element_text(face = 'bold', size = 42, margin = margin(r = 30)))+
...
mdl2 <- lmer(disp_V1_abs ~ med_con + focus_con + vowel + cons + age  + (1 + focus_con|speaker), data_red, REML = FALSE)
...
mdl3 <- lmer(pvel_V1_abs ~ med_con + focus_con + vowel + cons + age  + (1 + focus_con|speaker), data_red, REML = FALSE)
...
mdl4 <- lmer(onsC1_to_onsV1 ~ med_con + focus_con + vowel + cons + age  + (1 + focus_con|speaker), data_red, REML = FALSE)
...
mdl5 <- lmer(targC1_to_targV1 ~ med_con + focus_con + vowel + cons + age  + (1 + focus_con|speaker), data_red, REML = FALSE)
...
mdl6 <- lmer(onsV1.syll.dur ~ med_con + focus_con + vowel + cons + age  + (1 + focus_con|speaker), data_red, REML = FALSE)
...
mdl7 <- lmer(targV1.syll.dur ~ med_con + focus_con + vowel + cons + age + (1 + focus_con|speaker), data_red, REML = FALSE)
